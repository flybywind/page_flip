  ...
  var _bookObj = {
    ...
    _flip: function(){
      ...
      if (p.process > p.duration) {
        ...
        var back_rotate = "rotateY(" + (180 - ang) + "deg) translateZ(1px)",
            front_rotate;
        if (ang > 90) {
          front_rotate = "translateZ(0px)"
        } else {
          front_rotate = "rotateY(-" + ang + "deg) translateZ(1px)";
          if (ang > 10) {
            if (!p.next_tiny_angle) {
              p.next_tiny_angle = "rotateY(-" + ang + "deg) translateZ(1px)"
              var next_page = this.$current.nextAll(".page").eq(0);
              next_page.css({
                webkitTransform: p.next_tiny_angle,
                transform: p.next_tiny_angle,
              });
            }
          }
        }
        this.$front.css({
          webkitTransform: front_rotate,
          transform : front_rotate
        })
        this.$back.css({
          webkitTransform: back_rotate,
          transform : back_rotate
        });
